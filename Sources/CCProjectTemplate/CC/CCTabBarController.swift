    ...
    open func updateTabBarItem() {
        ...
        for vc in viewControllers ?? [] {
            ...
            if !item.image.isEmpty() {
                ...
                vc.tabBarItem.image = UIImage.init(named: item.image.number.string, in: item.imageModule.number.bool ? .module : .main, compatibleWith: nil)?.withRenderingMode(.alwaysOriginal)
            }
            
            if !item.selectedImage.isEmpty() {
                
                vc.tabBarItem.selectedImage = UIImage.init(named: item.selectedImage.number.string, in: item.selectedImageModule.number.bool ? .module : .main, compatibleWith: nil)?.withRenderingMode(.alwaysOriginal)
            }
